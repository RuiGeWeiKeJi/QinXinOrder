...
namespace Utility
{
    ...
    public class ImageZoomCut
    {
        ...
        /// <param name="fileSaveUrl">缩略图存放地址(jpg格式)</param>
            ...
            bool isDealSmall , string watermarkText, string watermarkImage)
        {
            ...
            if (initImage.Width <= targetWidth && initImage.Height <= targetHeight)
            {
                ...
                {// 按指定宽高 居中显示图片
                    ...
                    RectangleF dRect = new RectangleF();
                    dRect.X = (float)(targetWidth - initImage.Width) / 2;
                    dRect.Y = (float)(targetHeight - initImage.Height) / 2;
                    dRect.Width = (float)initImage.Width;
                    dRect.Height = (float)initImage.Height;

                    RectangleF sRect = new RectangleF(0, 0, initImage.Width, initImage.Height);
                   
                    DrawImage(initImage, newWidth, newHeight, dRect, sRect, Color.White, watermarkText, watermarkImage, savePath, ImageFormat.Jpeg);
            ...
            else
            {
                ...
                Rectangle dRect = new Rectangle ( 0,0, (int)newWidth , (int)newHeight );
                Rectangle sRect = new Rectangle ( 0,0, initImage.Width , initImage.Height );
        ...
        protected static void DrawImage(Image initImage , double newWidth , double newHeight , 
            RectangleF dRect , RectangleF sRect  , Color backgroudColor , 
            string watermarkText , string watermarkImage , string savePath , ImageFormat imageFormat )
        {
            //生成新图
            //新建一个bmp图片
            ...
            //设置背景色
            newG.Clear( backgroudColor );         
            //画图
            newG.DrawImage(initImage, dRect, sRect, GraphicsUnit.Pixel);                                   
            //文字水印
            DrawWatermarkText( newImage , watermarkText);              
            //透明图片水印
            DrawWatermarkImage(newImage, watermarkImage);  
            //保存缩略图
            newImage.Save(savePath, imageFormat);
            //释放资源                                 
            newG.Dispose();
            newImage.Dispose();
            //return newImage; 
        ...
        /// <param name="watermarkText"></param>
        protected static void DrawWatermarkText(Image newImage , string watermarkText)
        {
        ...
        {
            if (string.IsNullOrEmpty(watermarkImagePath)) return;   
            if (false == File.Exists(watermarkImagePath)) return;              
            //获取水印图片
